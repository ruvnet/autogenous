...
| 6 | **Cross-organization intelligence** — sovereign peers disclose signed findings + confidence + permitted evidence, no dataset pooling | `src/disclosure.ts` (`discloseFinding`/`verifyDisclosure`) — signed disclosure carrying claim + confidence + evidence digests ≤ privacy ceiling; raw payloads never cross | **Disclosure half BUILT** (`test/disclosure.test.ts`; signed + numeric-confidence disclosure is *ahead* of deployed CTA practice per research) — the **pooled cross-org training** half remains research (Swift 13-bank pilot, synthetic-only) |
...
| 9 | **A market for machine expertise** — advertise capability, publish signed performance history, earn reputation/payment on verified contribution | `src/reputation.ts` — signed `CapabilityClaim`s; a `PerformanceRecord` earns reputation ONLY from externally-verified contribution (mints only when `admitDurableWrite` admits, carrying the affirming verdicts as proof) | **Seed BUILT** (`test/reputation.test.ts`) — advertise + reputation-from-verified-contribution work. **Open**: the `selectionWeight` `w=q·t·r/(c·l)` form is an **unvalidated hypothesis** (research found zero backing — labeled in code); settlement/payment not built |
